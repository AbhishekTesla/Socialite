...
router.post("/",authMiddleware,async(req,res)=>{
    ...
    if(text.length < 1)
    return res.status(401).send("Text must be atleast 1 char");
...
router.get("/",authMiddleware,async(req,res)=>{
    ...
    try{
        ...
        const loggedUser = await FollowerModel.findOne({user:userId}); // as we only need the following from the followerModel
...
router.post("/like/:postId",authMiddleware,async(req,res)=>{
    try{
        ...
        const isLiked =
           ...
           //adds new items to the beginning of the original array and returns the new length
...
router.post("/comment/:postId",authMiddleware,async(req,res)=>{
    try{
        ...
        if(text.length < 1)
        return res.status(401).send("Comment Should be atleast 1 char");
